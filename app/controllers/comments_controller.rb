class CommentsController < ApplicationController
	...
	def destroy
		...
		if path.nil? || path == ''
			redirect_to project_blob_path(@project, blob)
		else
			path.gsub! '/','_'
			redirect_to project_parent_blob_path(@project, path, blob)
		end
	end
...
private
	...
	def log type
		...
		if path.nil? || path == ''
			url = project_blob_path @project, @comment.blob
		else
			path.gsub! '/','_'
			url = project_parent_blob_path @project, path, @comment.blob
		end
